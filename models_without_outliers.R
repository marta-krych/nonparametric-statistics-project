...
#plot of model 2
DPI_m.grid <- seq(min(reduced.no.rbd$DPI_m), 
                  max(reduced.no.rbd$DPI_m),
                  length.out = 100)

pfitM <- predict(model2, newdata=
                   data.frame(DPI_m = DPI_m.grid, Gender = "M"), type="response")

pfitF <- predict(model2, newdata=
                    data.frame(DPI_m = DPI_m.grid, Gender = "F"), type="response")

with(reduced.no.rbd, plot(DPI_m, y, xlim=range(DPI_m.grid), type ="n", 
                          ylab = "Parkinson risk",ylim=c(0,1)) )
points (jitter(reduced.no.rbd$DPI_m), y ,cex = 1, pch ="|",
        col =" darkgrey ")
lines(DPI_m.grid , pfitM ,lwd =2, col =" blue")
lines(DPI_m.grid , pfitF ,lwd =2, col =" pink")

legend('bottomright', c("Male", "Female"),
       lty=c(1,1), lwd=c(2,2), col=c("blue","pink"))
