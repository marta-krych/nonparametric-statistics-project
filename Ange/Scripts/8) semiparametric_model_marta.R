...
## Here i start with my modifications: conformal prediction
library(devtools)
...
train_gam <- function(x, y, out=NULL){
  colnames(x) <- c('var1','var2','var3','var4','var5','var6')
  train_data <- data.frame(y, x)
  model_gam <- gam(as.factor(y) ~ var1+var2+var3+ s(var4) + s(var5) + s(var6), family = "binomial", data=train_data)
}

predict_gam <- function(obj, new_x){
  new_x <- data.frame(new_x)
  colnames(new_x) <- c('var1','var2','var3','var4','var5','var6')
  predict.gam(obj, new_x, type = "response")
}



c_preds <- conformal.pred(cbind(df_model_g$Gender,df_model_g$RST_r, df_model_g$DPI_r, df_model_g$DUS_r, df_model_g$DPI_m,df_model_g$RST_m), df_model_g$Category, cbind(test$Gender,test$RST_r, test$DPI_r,test$DUS_r, test$DPI_m, test$RST_m), alpha=0.05, verbose=F, train.fun=train_gam, predict.fun=predict_gam, num.grid.pts=200)
c_preds

table(true_label=updrs_RBD, prediction=ifelse(c_preds$pred > .5, 1, 0))

cols <- ifelse(updrs_RBD == 1, 'red', 'black')
plot(c_preds$pred, col=cols, pch=16)
abline(0.5, 0, col='red', lty=2, lwd=2)

c_preds_split <- conformal.pred.split(cbind(df_model_g$Gender,df_model_g$RST_r, df_model_g$DPI_r, df_model_g$DUS_r, df_model_g$DPI_m,df_model_g$RST_m), df_model_g$Category, cbind(test$Gender,test$RST_r, test$DPI_r,test$DUS_r, test$DPI_m, test$RST_m), alpha=0.05,verbose=F, train.fun=train_gam, predict.fun=predict_gam)
c_preds_split

table(true_label=updrs_RBD, prediction=ifelse(c_preds_split$pred > .5, 1, 0))

cols <- ifelse(updrs_RBD == 1, 'red', 'black')
plot(c_preds_split$pred, col=cols, pch=16)
abline(0.5, 0, col='red', lty=2, lwd=2)
...
## The coputations are too heavy. Let's remove some features with LASSO:
#   - set the variables: 
x <- model.matrix( ~Gender+RST_r+DPI_r+DUS_r+DPI_m+RST_m)[,-1]
y <- as.factor(df_model_$Category)
...
df_model_g <- df_model_g[,c(1,2,4,7)]
with(df_model_g, scatterplotMatrix(data.frame(Category, Gender, DPI_r, DPI_m)))

# linear relationship: Gender, RST_r, RST_m
# non-linear relationship: DPI_r, DPI_m, DUS_r

model_semipar_g <- gam(Category ~ Gender + s(DPI_r) + s(DPI_m), 
                       family=binomial, data=df_model_g)
summary(model_semipar_g)
...
# No split:
c_preds <- conformal.pred(cbind(df_model_g$Gender, df_model_g$DPI_r, df_model_g$DPI_m), df_model_g$Category, cbind(test$Gender, test$DPI_r, test$DPI_m), alpha=0.05, verbose=F, train.fun=train_gam, predict.fun=predict_gam, num.grid.pts=200)
c_preds
...
# Split:
c_preds_split <- conformal.pred.split(cbind(df_model_g$Gender, df_model_g$DPI_r, df_model_g$DPI_m), df_model_g$Category, cbind(test$Gender, test$DPI_r, test$DPI_m), alpha=0.05,verbose=F, train.fun=train_gam, predict.fun=predict_gam)
c_preds_split
